...
mod windows_impl {
            ...
            match SetWindowsHookExW(WH_KEYBOARD_LL, Some(low_level_kbd_proc), hinst, 0) {
                Ok(handle) => {
                    *hook_handle().lock().unwrap() = handle.0;
                    ...
                    // Anchor event: any subsequent telemetry event in
                    // this subsystem can be cross-referenced against
                    // "did the hook ever install?" Critical for
                    // diagnosing "suppression doesn't work" reports —
                    // if this event is missing from the log stream,
                    // the hook never started; if present, the hook is
                    // running and the bug is elsewhere.
                    crate::services::telemetry::event(
                        crate::services::telemetry::Level::Info,
                        "raw_input_capture",
                        "WH_KEYBOARD_LL hook installed successfully",
                        serde_json::json!({}),
                    );
                }
                Err(e) => {
                    ...
                    // Promote hook-install failure to a structured event
                    // so we see it without waiting for the user to also
                    // reproduce a downstream symptom. Without the hook,
                    // suppression is fully disabled — that's a real
                    // production problem.
                    crate::services::telemetry::event(
                        crate::services::telemetry::Level::Error,
                        "raw_input_capture",
                        "raw_input_capture: SetWindowsHookExW failed — focus-stealing suppression disabled",
                        serde_json::json!({
                            "failure": "hook_install",
                            "error": format!("{e}"),
                        }),
                    );
    ...
    unsafe fn resolve_usb_id(h_device: isize) -> Option<UsbId> {
        ...
        // Also emit as a structured event so we can debug field
        // installations from the Loki side without asking users to
        // paste log lines. Volume is naturally bounded — at most one
        // event per (process, device handle), so a session usually
        // emits 2–6 of these total no matter how much typing/scanning
        // happens. Filter with LogQL: `| json | event_type="new_device_seen"`.
        //
        // PII note: the device path can contain a Bluetooth MAC address
        // for BT-paired devices. This was already flowing to our old
        // Sentry instance with `send_default_pii: true`. Same data,
        // different destination — we're not introducing a new PII
        // channel, and the destination is our own infrastructure now.
        crate::services::telemetry::event(
            crate::services::telemetry::Level::Info,
            "raw_input_capture",
            ...
            serde_json::json!({
                ...
                "is_managed": is_managed_at_parse_time,
                "device_path": name,
                "parsed_vid_pid": match parsed {
                    Some(id) => format!("VID=0x{:04X} PID=0x{:04X}", id.vid, id.pid),
                    None => "None (parser found no recognized format)".to_string(),
                },
                "managed_ids_count": managed_ids().lock().unwrap().len() as i64,
            }),
        );
